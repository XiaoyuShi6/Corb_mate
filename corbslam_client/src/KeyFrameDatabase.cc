 ...
 */
    vector<KeyFrame*> KeyFrameDatabase::DetectMapFusionCandidatesFromDB(KeyFrame *pKF) {
        ...
        //mvInvertedFile vector<list<keyFrame*>> 包含word的关键帧
            // words是检测图像是否匹配的枢纽，遍历该pKF的每一个word
            for (DBoW2::BowVector::const_iterator vit = pKF->mBowVec.begin(), vend = pKF->mBowVec.end();
                 vit != vend; vit++) {
                //包含mvInvertedFile[vit->first]序号代表的word的关键帧序列
                ...
                for (list<LightKeyFrame>::iterator lit = lKFs.begin(), lend = lKFs.end(); lit != lend; lit++) {
                    KeyFrame *pKFi = (*lit).getKeyFrame();
                    if( pKFi ) {
                        if (pKFi->mnRelocQuery != pKF->mnId) { // pKFi还没有标记为pKF的候选帧
                            pKFi->mnRelocWords = 0;
                            pKFi->mnRelocQuery = pKF->mnId;   //标记,之后直接跳过
                            lKFsSharingWords.push_back(pKFi);
                        }
                        pKFi->mnRelocWords++;//和该帧共同含有的单词数
                    }
                }
            }
        }
